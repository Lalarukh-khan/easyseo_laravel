            ...
            <div class="col-xl-6 col-lg-6 col-6">
                ...
                <br>
                    ...
                    <!-- <textarea name="" id="first_result_div" cols="30" rows="18" class="form-control" style=" color: black; "></textarea> -->
                        ...
                        <div class="row rsltdwholebox">
                                    <div class="col-lg-11">
                                        <div name="" id="first_result_div1" class="frstrsltdiv"></div>
                                    </div>
                                    <div class="col-lg-1" style="padding: 0px !important;">
                                        <div id="resulted_phrase1"></div>
                                    </div>
                                    <textarea name="content1" id="details1" cols="30" rows="10" style="display: none !important;"></textarea>
                                    {{--<form action="{{route('user.template.form_seo_score')}}" method="post">--}}
                                    <form id="seo_content_form1" style="display: none !important;">
                                        @csrf
                                        <input type="text" name="score1" id="formscore1">
                                        <input type="text" name="temp_id1" id="temp_id1">
                                    </form>
                        </div>
                       {{-- <div class="mt-2"
                            style=" padding: 5px 7px 3px; border-radius: 12px; background: #ececf1; color: #565869;float: right;">
                            <div id="prompt_word_count1">0</div>
                        </div> --}} 
                        <div class="row rsltdwholebox">
                                    <div class="col-lg-11">
                                        <div name="" id="first_result_div2" class="frstrsltdiv"></div>
                                    </div>
                                    <div class="col-lg-1" style="padding: 0px !important;">
                                        <div id="resulted_phrase2"></div>
                                    </div>
                                    <textarea name="content2" id="details2" cols="30" rows="10" style="display: none !important;"></textarea>
                                    {{--<form action="{{route('user.template.form_seo_score')}}" method="post">--}}
                                    <form id="seo_content_form2" style="display: none !important;">
                                        @csrf
                                        <input type="text" name="score2" id="formscore2">
                                        <input type="text" name="temp_id2" id="temp_id2">
                                    </form>
                        </div>
                       {{-- <div class="mt-2"
                            style=" padding: 5px 7px 3px; border-radius: 12px; background: #ececf1; color: #565869;float: right;">
                            <div id="prompt_word_count2">0</div>
                        </div> --}} 
		...
		.then(data => {
            ...
            const finallist = this.list;
                    ...
                    for (let j = 0; j < item.response.tasks.length; j++) {
                        ...
                        let seoScore = (keywordDensity * 10) + (metaTagsScore * 3) + (automated_readability_index * 3) + (smog_readability_index * 3);
                        let roundedscore = Math.round(seoScore);
                        document.getElementById("resulted_phrase1").innerHTML = roundedscore;
                        document.getElementById("formscore1").value = roundedscore;
                        var seoform = document.getElementById('seo_content_form1');
                        var seoformData = new FormData(seoform);
                        $.ajax({
                            url: "{{ route('user.template.seo_form_submit') }}",
                            method: "POST",
                        ...
                        const numberEl1 = document.getElementById("resulted_phrase1");
                        const number1 = parseInt(numberEl1.textContent);

                        numberEl1.style.borderRadius = "50%";
                        numberEl1.style.padding = "6px 8px";
                        numberEl1.style.fontSize = "15px";
                        numberEl2.style.textAlign = "center";

                        if (number1 < 50) {
                        numberEl1.style.border = "2px solid #f54c36";
                        }
                        else if (number1 >= 50 && number1 <= 70) {
                        numberEl1.style.border = "2px solid #f7831e";
                        }
                        else {
                        numberEl1.style.border = "2px solid #39942f";
                        }
                        // html += `<p class="gnrtdtext">${generated_text}</p>`;
                        }
                    }
		...
		.then(data => {
            ...
            const finallist = this.list;
                    ...
                    for (let j = 0; j < item.response.tasks.length; j++) {
                        ...
                        let seoScore = (keywordDensity * 10) + (metaTagsScore * 3) + (automated_readability_index * 3) + (smog_readability_index * 3);
                        let roundedscore = Math.round(seoScore);
                        document.getElementById("resulted_phrase2").innerHTML = roundedscore;
                        document.getElementById("formscore2").value = roundedscore;
                        var seoform = document.getElementById('seo_content_form2');
                        var seoformData = new FormData(seoform);
                        $.ajax({
                            url: "{{ route('user.template.seo_form_submit') }}",
                            method: "POST",
                        ...
                        const numberEl2 = document.getElementById("resulted_phrase2");
                        const number2 = parseInt(numberEl2.textContent);

                        numberEl2.style.borderRadius = "50%";
                        numberEl2.style.padding = "6px 8px";
                        numberEl2.style.fontSize = "15px";
                        numberEl2.style.textAlign = "center";

                        if (number2 < 50) {
                        numberEl2.style.border = "2px solid #f54c36";
                        }
                        else if (number2 >= 50 && number2 <= 70) {
                        numberEl2.style.border = "2px solid #f7831e";
                        }
                        else {
                        numberEl2.style.border = "2px solid #39942f";
                        }
                        // html += `<p class="gnrtdtext">${generated_text}</p>`;
                        }
                    }
                ...
                success: function(data) {
                    ...
                    if (data.status == 400) {
                        ...
                        $.ajax({
                            ...
                            processData: false,
                            success: function(data) {
                                document.getElementById("first_result_div1").innerHTML = data.message;
                                document.getElementById("details1").value = data.message;
                                $('#prompt_word_count1').html(`${data.word_count} words`);
                                const score = data.message;
                                getSeoScore1(score);
                                document.getElementById("temp_id1").value = data.temp_id;
                                $.ajax({
                                    url: "{{ route('user.template.form_submit') }}",
                                    method: "POST",
                                    data: formData,
                                    ...
                                    success: function(data) {
                                        templateLoader('#ai-loader','hide');
                                        $('#ans_div').show();
                                        document.getElementById("first_result_div2").innerHTML = data.message;
                                        document.getElementById("details2").value = data.message;
                                        $('#prompt_word_count2').html(`${data.word_count} words`);
                                        const score = data.message;
                                        getSeoScore2(score);
                                        document.getElementById("temp_id2").value = data.temp_id;
                                    }
                                });
                            }
                        });

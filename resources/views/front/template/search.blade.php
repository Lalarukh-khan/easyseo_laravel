...
<form action="{{ route('user.template.search') }}">
    <div class="flex-grow-1 search-bar">
        <div class="input-group">
            ...
            <input type="text" name="search" class="form-control" id="nwtempsearchbar" placeholder="What do you want to create?" value="{{$searchtext ?? ''}}"/>
        </div> 
    </div> 
</form>
<br>
...
<div class="row response-row">
    @forelse ($search as $ser)
    @if ($ser->count() > 0)
	<div class="col-lg-3">
		<div class="row append_cat_{{$ser->id}}">
			<div class="col-lg-12 col-md-12 m-2">
				<h5 class="nwtmpcatname" style="font-size: 1.3rem">{{$ser->name}} </h5>
			</div>
			<div class="col-xl-12 col-lg-12 col-md-12 col-sm-12">
				<a href="{{route('user.template.make',$ser->hashid)}}">
					<!-- <div class="card checked nwcard" style="border-radius: 15px; min-height: 250px;"> -->
					<div class="card checked nwcard" >
						{{-- <img src="{{check_file($ser->icon,'user')}}" class="mt-4 ml-3 nwtempcatimg" width="50px" alt="template logo"
							style="margin-left: 15px;"> --}}
						<img src="{{asset($ser->icon)}}" class="mt-4 ml-3 nwtempcatimg" width="50px" alt="template logo"
                        style="margin-left: 15px;">
						<div class="card-body text-dark">
							<h5 class="card-title nwcard-title">{{$ser->name}}</h5>
							<p class="card-text nwcard-text">{{Str::limit($ser->description,150,$end='...')}}</p>
						</div>
					</div>
				</a>
			</div>
    ...
    <div class="row mt-5 p-5">
        <div class="col-xl-12 col-lg-12 mb-4 col-md-12 col-sm-12 mt-5" style="padding: 0.2rem!important;">
            <center>
